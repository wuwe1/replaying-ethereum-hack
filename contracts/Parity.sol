...
contract WalletLibrary is WalletEvents {
    ...
    function initDaylimit(uint256 _limit) only_uninitialized {
        m_dailyLimit = _limit;
        m_lastDay = today();
    }
    ...
    function create(uint256 _value, bytes _code)
        internal
    ...
    function underLimit(uint256 _value) internal onlyowner returns (bool) {
...
contract Wallet is WalletEvents {
    ...
    function Wallet(
        ...
        uint256 argarraysize = (2 + _owners.length);
        uint256 argsize = (2 + argarraysize) * 32;

        // produces compiler bug for some reason
        // assembly {
    ...
    // Gets an owner by 0-indexed position (using numOwners as the count)
    function getOwner(uint256 ownerIndex) constant returns (address) {
        return address(m_owners[ownerIndex + 1]);
    }
