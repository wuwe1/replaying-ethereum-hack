...
library SushiswapV2Library {
    ...
    function getReserves(
        ...
        (uint256 reserve0, uint256 reserve1, ) =
            ISushiswapV2Pair(pairFor(factory, tokenA, tokenB)).getReserves();
...
contract StableYieldCredit is ReentrancyGuard {
    ...
    event Earned(address indexed from, uint256 amount);
    ...
    Oracle public constant LINK =
        Oracle(0x271bf4568fb737cc2e6277e9B1EE0034098cDA2a);
    ISushiswapV2Factory public constant FACTORY =
        ISushiswapV2Factory(0xC0AEe478e3658e2610c5F7A4A2E1777cE9e4f2Ac);

    // user => token => collateral
    mapping(address => mapping(address => uint256)) public collateral;
    // user => token => credit
    mapping(address => mapping(address => uint256)) public collateralCredit;
    ...
    function notifyFeeAmount(uint256 reward) internal updateReward(address(0)) {
        ...
        )
    {
        ...
        (uint256 reserveA, uint256 reserveB) =
            SushiswapV2Library.getReserves(address(FACTORY), tokenA, tokenB);
        if (reserveA == 0 && reserveB == 0) {
            (amountA, amountB) = (amountADesired, amountBDesired);
        } else {
            uint256 amountBOptimal =
                SushiswapV2Library.quote(amountADesired, reserveA, reserveB);
    ...
    function _withdraw(IERC20 token, uint256 amount) internal {
        uint256 _credit = collateralCredit[msg.sender][address(token)];
        uint256 _collateral = collateral[msg.sender][address(token)];
        ...
        uint256 _burned = (_collateral * amount) / _credit;
        ...
        (address _token0, ) =
            SushiswapV2Library.sortTokens(address(token), address(this));
